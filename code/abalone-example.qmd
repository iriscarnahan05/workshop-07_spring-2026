...
# here: allows data to be read in another folder within the file
# xlsx: reading an excel file
...
H~0~: **We will observe no change in abalone growth due to pH changes.**  

H~A~: **We will observe a change in abalone growth due to pH changes.**
...
# creating new object from `abalone`
...
ggplot(data = abalone_clean, # starting data frame
       aes(x = mean_ph, # x-axis: mean ph, y-axis: change in shell surface area
           y = change_in_area)) +
...
**The missing value from a pait of tanks in which the pH sensor malfunctioned, so there is no observation.**  
...
**`x`, `predicted`, `std.error`, `conf.high`, `group`**  
...
**At pH 8, the model predicts abalone shell growth is 5.36 [95% CI: 5.08, 5.64] mm^2^ d^-1^.**
